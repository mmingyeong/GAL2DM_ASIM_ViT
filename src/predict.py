# models/vit/predict.py
# -*- coding: utf-8 -*-

from __future__ import annotations

import os
import sys
import re
import argparse
from contextlib import nullcontext
from typing import List, Dict, Any

import numpy as np
import torch
import h5py
from tqdm import tqdm

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../")))

from src.data_loader import get_dataloader
from src.model import VoxelViTUNet3D as ViT3D
from src.logger import get_logger

logger = get_logger("predict_vitunet3d")


def str2bool(v):
    return str(v).lower() in ("1", "true", "t", "yes", "y")


def select_inputs(x: torch.Tensor, case: str, keep_two: bool) -> torch.Tensor:
    assert x.ndim == 5 and x.size(1) == 2, f"Expected [B,2,D,H,W], got {tuple(x.shape)}"

    if case == "both":
        return x

    if case == "ch1":
        ch1 = x[:, 0:1]
        if keep_two:
            return torch.cat([ch1, torch.zeros_like(ch1)], dim=1)
        return ch1

    if case == "ch2":
        ch2 = x[:, 1:2]
        if keep_two:
            return torch.cat([torch.zeros_like(ch2), ch2], dim=1)
        return ch2

    raise ValueError(f"Unknown input_case: {case}")


def _get_effective_file_paths_from_loader(loader) -> List[str]:
    ds = loader.dataset

    if hasattr(ds, "dataset") and hasattr(ds, "indices"):
        base = ds.dataset
        indices = list(ds.indices)

        if hasattr(base, "file_paths"):
            base_paths = list(base.file_paths)
            return [base_paths[i] for i in indices]

        for attr in ("files", "paths", "file_list"):
            if hasattr(base, attr):
                base_paths = list(getattr(base, attr))
                return [base_paths[i] for i in indices]

        raise AttributeError("Subset base dataset does not expose file paths.")

    if hasattr(ds, "file_paths"):
        return list(ds.file_paths)

    for attr in ("files", "paths", "file_list"):
        if hasattr(ds, attr):
            return list(getattr(ds, attr))

    raise AttributeError("Dataset does not expose file paths.")


def _load_checkpoint(model_path: str, device: torch.device):
    try:
        state = torch.load(model_path, map_location=device, weights_only=True)
    except TypeError:
        state = torch.load(model_path, map_location=device)
    except Exception as e:
        logger.warning(f"weights_only load failed with {e}; falling back to standard torch.load")
        state = torch.load(model_path, map_location=device)

    if isinstance(state, dict):
        if "state_dict" in state and isinstance(state["state_dict"], dict):
            state = state["state_dict"]
        elif "model" in state and isinstance(state["model"], dict):
            state = state["model"]
        elif "model_state_dict" in state and isinstance(state["model_state_dict"], dict):
            state = state["model_state_dict"]

    if isinstance(state, dict):
        cleaned = {}
        for k, v in state.items():
            nk = k[7:] if k.startswith("module.") else k
            cleaned[nk] = v
        state = cleaned

    return state


def _infer_arch_from_ckpt_name(model_path: str) -> Dict[str, Any]:
    name = os.path.basename(model_path)
    full_path = model_path
    out: Dict[str, Any] = {}

    m = re.search(r"_ps(\d+)x(\d+)", name)
    if m:
        out["image_patch_size"] = int(m.group(1))
        out["frame_patch_size"] = int(m.group(2))

    m = re.search(r"_st(\d+)x(\d+)", name)
    if m:
        out["image_patch_stride"] = int(m.group(1))
        out["frame_patch_stride"] = int(m.group(2))

    m = re.search(r"_dim(\d+)", name)
    if m:
        out["emb_dim"] = int(m.group(1))

    m = re.search(r"_depth(\d+)", name)
    if m:
        out["depth"] = int(m.group(1))

    m = re.search(r"_heads(\d+)", name)
    if m:
        out["heads"] = int(m.group(1))

    m = re.search(r"_mlp(\d+)", name)
    if m:
        out["mlp_dim"] = int(m.group(1))

    m = re.search(r"icase-(both|ch1|ch2)", name)
    if m:
        out["input_case"] = m.group(1)

    if "keep2" in name:
        out["keep_two_channels"] = True

    if "emb_dim" in out and "mlp_dim" not in out:
        out["mlp_dim"] = out["emb_dim"] * 2

    if "VIT_BASE32_MATCH" in full_path:
        out["vit_encoder_channels"] = [32, 64, 128]
        out["vit_decoder_channels"] = [256, 128, 64]
    elif "VIT_BASE64_MATCH" in full_path:
        out["vit_encoder_channels"] = [64, 128, 256]
        out["vit_decoder_channels"] = [512, 256, 128]
    elif "VIT_BASE128_MATCH" in full_path:
        out["vit_encoder_channels"] = [128, 256, 512]
        out["vit_decoder_channels"] = [1024, 512, 256]

    return out


def _resolve_model_config_from_ckpt(
    model_path: str,
    image_patch_size: int,
    frame_patch_size: int,
    image_patch_stride: int | None,
    frame_patch_stride: int | None,
    emb_dim: int,
    depth: int,
    heads: int,
    mlp_dim: int,
    input_case: str,
    keep_two_channels: bool,
    vit_encoder_channels: List[int] | None,
    vit_decoder_channels: List[int] | None,
    vit_dropout: float,
    vit_dim_head: int,
) -> Dict[str, Any]:

    inferred = _infer_arch_from_ckpt_name(model_path)
    ckpt_name = os.path.basename(model_path)

    defaults = {
        "image_patch_size": 16,
        "frame_patch_size": 16,
        "image_patch_stride": None,
        "frame_patch_stride": None,
        "emb_dim": 256,
        "depth": 3,
        "heads": 8,
        "mlp_dim": 512,
        "input_case": "both",
        "keep_two_channels": False,
        "vit_encoder_channels": None,
        "vit_decoder_channels": None,
        "vit_dropout": 0.1,
        "vit_dim_head": 64,
    }

    resolved = {
        "image_patch_size": image_patch_size,
        "frame_patch_size": frame_patch_size,
        "image_patch_stride": image_patch_stride,
        "frame_patch_stride": frame_patch_stride,
        "emb_dim": emb_dim,
        "depth": depth,
        "heads": heads,
        "mlp_dim": mlp_dim,
        "input_case": input_case,
        "keep_two_channels": keep_two_channels,
        "vit_encoder_channels": vit_encoder_channels,
        "vit_decoder_channels": vit_decoder_channels,
        "vit_dropout": vit_dropout,
        "vit_dim_head": vit_dim_head,
    }

    for k, v in inferred.items():
        if k in resolved and resolved[k] == defaults[k]:
            resolved[k] = v

    if resolved["mlp_dim"] == defaults["mlp_dim"] and resolved["emb_dim"] != defaults["emb_dim"]:
        resolved["mlp_dim"] = resolved["emb_dim"] * 2

    if resolved["vit_encoder_channels"] is None:
        resolved["vit_encoder_channels"] = [32, 64, 128]

    if resolved["vit_decoder_channels"] is None:
        resolved["vit_decoder_channels"] = [256, 128, 64]

    if len(resolved["vit_encoder_channels"]) != 3:
        raise ValueError(f"--vit_encoder_channels must have 3 values, got {resolved['vit_encoder_channels']}")

    if len(resolved["vit_decoder_channels"]) != 3:
        raise ValueError(f"--vit_decoder_channels must have 3 values, got {resolved['vit_decoder_channels']}")

    logger.info(f"🧩 Checkpoint name: {ckpt_name}")
    logger.info(f"🧩 Inferred from checkpoint/path: {inferred if inferred else 'nothing'}")
    logger.info(f"🧩 Resolved model config: {resolved}")

    return resolved


def run_prediction(
    yaml_path: str,
    output_dir: str,
    model_path: str,
    device: str = "cuda",
    batch_size: int = 1,
    image_size: int = 128,
    frames: int = 128,
    image_patch_size: int = 16,
    frame_patch_size: int = 16,
    image_patch_stride: int | None = None,
    frame_patch_stride: int | None = None,
    emb_dim: int = 256,
    depth: int = 3,
    heads: int = 8,
    mlp_dim: int = 512,
    vit_dim_head: int = 64,
    vit_encoder_channels: List[int] | None = None,
    vit_decoder_channels: List[int] | None = None,
    vit_dropout: float = 0.1,
    amp: bool = False,
    sample_fraction: float = 1.0,
    sample_seed: int = 42,
    input_case: str = "both",
    keep_two_channels: bool = False,
    validate_keys: bool = True,
    target_field: str = "rho",
    exclude_list: str | None = None,
    include_list: str | None = None,
    normalize_input: bool = True,
    normalize_target: bool = False,
    eps: float = 1e-12,
):
    if not (0 < sample_fraction <= 1.0):
        raise ValueError(f"--sample_fraction must be in (0,1], got {sample_fraction}")

    resolved = _resolve_model_config_from_ckpt(
        model_path=model_path,
        image_patch_size=image_patch_size,
        frame_patch_size=frame_patch_size,
        image_patch_stride=image_patch_stride,
        frame_patch_stride=frame_patch_stride,
        emb_dim=emb_dim,
        depth=depth,
        heads=heads,
        mlp_dim=mlp_dim,
        input_case=input_case,
        keep_two_channels=keep_two_channels,
        vit_encoder_channels=vit_encoder_channels,
        vit_decoder_channels=vit_decoder_channels,
        vit_dropout=vit_dropout,
        vit_dim_head=vit_dim_head,
    )

    image_patch_size = resolved["image_patch_size"]
    frame_patch_size = resolved["frame_patch_size"]
    image_patch_stride = resolved["image_patch_stride"]
    frame_patch_stride = resolved["frame_patch_stride"]
    emb_dim = resolved["emb_dim"]
    depth = resolved["depth"]
    heads = resolved["heads"]
    mlp_dim = resolved["mlp_dim"]
    input_case = resolved["input_case"]
    keep_two_channels = resolved["keep_two_channels"]
    vit_encoder_channels = tuple(resolved["vit_encoder_channels"])
    vit_decoder_channels = tuple(resolved["vit_decoder_channels"])
    vit_dropout = float(resolved["vit_dropout"])
    vit_dim_head = int(resolved["vit_dim_head"])

    case_suffix = f"icase-{input_case}{'-keep2' if keep_two_channels else ''}"
    output_dir = os.path.join(output_dir, case_suffix)
    os.makedirs(output_dir, exist_ok=True)

    dev = torch.device(device)

    DHW = (frames, image_size, image_size)
    patch_size_3d = (frame_patch_size, image_patch_size, image_patch_size)

    f_stride = frame_patch_stride if frame_patch_stride is not None else frame_patch_size
    i_stride = image_patch_stride if image_patch_stride is not None else image_patch_size
    patch_stride_3d = (f_stride, i_stride, i_stride)

    for s, p in zip(patch_stride_3d, patch_size_3d):
        if s > p:
            raise ValueError(f"Stride {patch_stride_3d} must be <= patch {patch_size_3d} per axis.")

    if input_case == "both":
        in_ch = 2
    else:
        in_ch = 2 if keep_two_channels else 1

    logger.info("📦 Building model VoxelViTUNet3D ...")

    model_kwargs = dict(
        image_size=DHW,
        in_channels=in_ch,
        out_channels=1,
        patch_size=patch_size_3d,
        patch_stride=patch_stride_3d,
        dim=emb_dim,
        depth=depth,
        heads=heads,
        mlp_dim=mlp_dim,
        encoder_channels=vit_encoder_channels,
        decoder_channels=vit_decoder_channels,
        dropout=vit_dropout,
    )

    try:
        model = ViT3D(**model_kwargs, dim_head=vit_dim_head).to(dev)
    except TypeError:
        logger.warning("ViT3D does not accept dim_head; rebuilding without dim_head.")
        model = ViT3D(**model_kwargs).to(dev)

    logger.info(
        f"🧱 Model: image_size={DHW}, patch_size={patch_size_3d}, stride={patch_stride_3d}, "
        f"dim={emb_dim}, depth={depth}, heads={heads}, mlp_dim={mlp_dim}, dim_head={vit_dim_head}, "
        f"encoder_channels={vit_encoder_channels}, decoder_channels={vit_decoder_channels}, "
        f"dropout={vit_dropout}, in_channels={in_ch}, input_case={input_case}, keep_two={keep_two_channels}"
    )

    logger.info(f"📥 Loading checkpoint: {model_path}")
    state = _load_checkpoint(model_path, dev)
    missing, unexpected = model.load_state_dict(state, strict=False)

    if missing:
        logger.warning(f"Missing keys while loading: {missing}")
    if unexpected:
        logger.warning(f"Unexpected keys while loading: {unexpected}")

    model.eval()

    augmentation_cfg = {"enable": False}
    normalization_cfg = {
        "mode": "custom" if (normalize_input or normalize_target) else "none",
        "normalize_input": bool(normalize_input),
        "normalize_target": bool(normalize_target),
        "eps": float(eps),
    }

    test_loader = get_dataloader(
        yaml_path=yaml_path,
        split="test",
        batch_size=batch_size,
        shuffle=False,
        sample_fraction=sample_fraction,
        num_workers=0,
        pin_memory=True,
        target_field=target_field,
        dtype=torch.float32,
        seed=sample_seed,
        train_val_split=0.8,
        validate_keys=validate_keys,
        strict=False,
        exclude_list_path=exclude_list,
        include_list_path=include_list,
        augmentation=augmentation_cfg,
        normalization=normalization_cfg,
        apply_augmentation_in=(),
    )

    file_paths = _get_effective_file_paths_from_loader(test_loader)
    assert len(file_paths) == len(test_loader.dataset)

    logger.info(f"🧪 Test samples: {len(test_loader.dataset)}")
    logger.info(f"🧮 Normalization config: {normalization_cfg}")

    try:
        autocast_ctx = (
            torch.amp.autocast(device_type="cuda", dtype=torch.float16)
            if (amp and dev.type == "cuda")
            else torch.amp.autocast(device_type="cpu", dtype=torch.bfloat16)
            if (amp and dev.type == "cpu")
            else nullcontext()
        )
    except Exception:
        from torch.cuda.amp import autocast as legacy_autocast
        autocast_ctx = legacy_autocast(enabled=amp)

    saved_files = []
    skipped_files = []
    sample_offset = 0

    torch.set_grad_enabled(False)

    with torch.no_grad():
        for batch_idx, (x, _y_unused) in enumerate(tqdm(test_loader, desc="🚀 Running ViT predictions")):
            batch_size_actual = x.size(0)

            x = x.to(dev, non_blocking=True)
            x = select_inputs(x, input_case, keep_two_channels)

            if x.size(1) != in_ch:
                raise RuntimeError(f"Post-selection channels {x.size(1)} != model in_channels {in_ch}")

            if tuple(x.shape[-3:]) != DHW:
                raise ValueError(f"Input spatial {tuple(x.shape[-3:])} != expected {DHW}")

            with autocast_ctx:
                pred = model(x)

            y_pred = pred.float().cpu().numpy()
            y_pred = np.squeeze(y_pred, axis=1)

            batch_file_paths = file_paths[sample_offset: sample_offset + batch_size_actual]

            for b, src_path in enumerate(batch_file_paths):
                filename = os.path.basename(src_path)
                output_path = os.path.join(output_dir, filename)

                if os.path.exists(output_path):
                    logger.info(f"[SKIP] Already exists: {output_path}")
                    skipped_files.append(output_path)
                    continue

                pred_i = y_pred[b]

                with h5py.File(output_path, "w") as f_out:
                    f_out.create_dataset("prediction", data=pred_i, compression="gzip")

                    f_out.attrs["source_file"] = src_path
                    f_out.attrs["model_path"] = model_path
                    f_out.attrs["model_class"] = model.__class__.__name__
                    f_out.attrs["amp"] = bool(amp)

                    f_out.attrs["image_size"] = int(image_size)
                    f_out.attrs["frames"] = int(frames)
                    f_out.attrs["image_patch_size"] = int(image_patch_size)
                    f_out.attrs["frame_patch_size"] = int(frame_patch_size)
                    f_out.attrs["image_patch_stride"] = int(i_stride)
                    f_out.attrs["frame_patch_stride"] = int(f_stride)
                    f_out.attrs["emb_dim"] = int(emb_dim)
                    f_out.attrs["depth"] = int(depth)
                    f_out.attrs["heads"] = int(heads)
                    f_out.attrs["mlp_dim"] = int(mlp_dim)
                    f_out.attrs["vit_dim_head"] = int(vit_dim_head)
                    f_out.attrs["vit_encoder_channels"] = str(tuple(vit_encoder_channels))
                    f_out.attrs["vit_decoder_channels"] = str(tuple(vit_decoder_channels))
                    f_out.attrs["vit_dropout"] = float(vit_dropout)

                    f_out.attrs["sample_fraction_files_only"] = float(sample_fraction)
                    f_out.attrs["sample_seed"] = int(sample_seed)
                    f_out.attrs["input_case"] = str(input_case)
                    f_out.attrs["keep_two_channels"] = bool(keep_two_channels)

                    f_out.attrs["normalization_mode"] = str(normalization_cfg["mode"])
                    f_out.attrs["normalize_input"] = bool(normalization_cfg["normalize_input"])
                    f_out.attrs["normalize_target"] = bool(normalization_cfg["normalize_target"])
                    f_out.attrs["eps"] = float(normalization_cfg["eps"])

                    f_out.attrs["batch_index"] = int(batch_idx)
                    f_out.attrs["batch_pos"] = int(b)
                    f_out.attrs["global_sample_index"] = int(sample_offset + b)

                saved_files.append(output_path)

            sample_offset += batch_size_actual

    logger.info("====== ViT Inference Summary ======")
    logger.info(f"Saved files   : {len(saved_files)}")
    logger.info(f"Skipped files : {len(skipped_files)}")
    logger.info(f"Total handled : {sample_offset}")

    if saved_files:
        logger.info("Saved first 5: " + ", ".join(os.path.basename(p) for p in saved_files[:5]))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run VoxelViTUNet3D inference on A-SIM test split."
    )

    parser.add_argument("--yaml_path", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--model_path", type=str, required=True)

    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--batch_size", type=int, default=1)
    parser.add_argument("--amp", action="store_true")

    parser.add_argument("--image_size", type=int, default=128)
    parser.add_argument("--frames", type=int, default=128)
    parser.add_argument("--image_patch_size", type=int, default=16)
    parser.add_argument("--frame_patch_size", type=int, default=16)
    parser.add_argument("--image_patch_stride", type=int, default=None)
    parser.add_argument("--frame_patch_stride", type=int, default=None)

    parser.add_argument("--emb_dim", type=int, default=256)
    parser.add_argument("--depth", type=int, default=3)
    parser.add_argument("--heads", type=int, default=8)
    parser.add_argument("--mlp_dim", type=int, default=512)

    parser.add_argument("--vit_dim_head", type=int, default=64)
    parser.add_argument("--vit_encoder_channels", type=int, nargs=3, default=None)
    parser.add_argument("--vit_decoder_channels", type=int, nargs=3, default=None)
    parser.add_argument("--vit_dropout", type=float, default=0.1)

    parser.add_argument("--sample_fraction", type=float, default=1.0)
    parser.add_argument("--sample_seed", type=int, default=42)

    parser.add_argument("--input_case", type=str, choices=["both", "ch1", "ch2"], default="both")
    parser.add_argument("--keep_two_channels", action="store_true")

    parser.add_argument("--validate_keys", type=str2bool, default=True)
    parser.add_argument("--exclude_list", type=str, default=None)
    parser.add_argument("--include_list", type=str, default=None)

    parser.add_argument("--target_field", type=str, choices=["rho", "tscphi"], default="rho")
    parser.add_argument("--normalize_input", type=str2bool, default=True)
    parser.add_argument("--normalize_target", type=str2bool, default=False)
    parser.add_argument("--eps", type=float, default=1e-12)

    args = parser.parse_args()

    run_prediction(
        yaml_path=args.yaml_path,
        output_dir=args.output_dir,
        model_path=args.model_path,
        device=args.device,
        batch_size=args.batch_size,
        image_size=args.image_size,
        frames=args.frames,
        image_patch_size=args.image_patch_size,
        frame_patch_size=args.frame_patch_size,
        image_patch_stride=args.image_patch_stride,
        frame_patch_stride=args.frame_patch_stride,
        emb_dim=args.emb_dim,
        depth=args.depth,
        heads=args.heads,
        mlp_dim=args.mlp_dim,
        vit_dim_head=args.vit_dim_head,
        vit_encoder_channels=args.vit_encoder_channels,
        vit_decoder_channels=args.vit_decoder_channels,
        vit_dropout=args.vit_dropout,
        amp=args.amp,
        sample_fraction=args.sample_fraction,
        sample_seed=args.sample_seed,
        input_case=args.input_case,
        keep_two_channels=args.keep_two_channels,
        validate_keys=args.validate_keys,
        target_field=args.target_field,
        exclude_list=args.exclude_list,
        include_list=args.include_list,
        normalize_input=args.normalize_input,
        normalize_target=args.normalize_target,
        eps=args.eps,
    )